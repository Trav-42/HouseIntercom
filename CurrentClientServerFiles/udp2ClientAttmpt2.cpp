 ...
 */
 ...
 static uint8_t rec_record_idx  = 2; // on 3/28 changed from size_t to uint8_t so it can be transmitted via UDP
...
void setup()
{
    ...
    StickCP2.Display.drawString("Client", 120, 3);
...
void loop()
{
   ...
   // Increase noise filter level, and show the device is a "Client" device
    if (StickCP2.BtnB.wasHold()) {
        ...
        if (StickCP2.Speaker.isEnabled()) {
            ...
            udpMessage.write(rec_data, record_size); // write the data into a message packet
            udp.broadcastTo(&rec_record_idx, sizeof(rec_record_idx), 1234); // Send the rec_idx
            udp.broadcastTo(udpMessage, 1234); // Broadcast the data
            udpMessage.flush(); // Clear the message class
